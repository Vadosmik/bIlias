...
export default function CoursesPage() {
  ...
  const filteredCourses = useMemo(() => {
    return courses.filter(course => {
      ...
      const matchesDept = selectedDept === departments[0] || course.department === selectedDept;
      const matchesSem = selectedSem === semesters[0] || course.semester.toString() === selectedSem;
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <p className="text-muted-foreground mt-2">Przeglądaj i dołączaj do dostępnych kursów na Twoim wydziale.</p>
      </section>
      ...
      <div className="bg-white p-4 rounded-2xl border border-brand-gray/10 shadow-brand-sm flex flex-col md:flex-row gap-4 items-center">
        ...
        >
          ...
          {semesters.slice(1).map(sem => <option key={sem} value={sem}>Semestr {sem}</option>)}
        </select>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-brand-gray/5 bg-brand-light/50">
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">Kurs</th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground hidden lg:table-cell">Wydział</th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground text-center">Semestr</th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">Prowadzący</th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground text-right">Akcja</th>
            </tr>
          </thead>
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-col gap-1">
                    {course.lecturers.map(lecturer => (
                      <div key={lecturer} className="flex items-center gap-1.5 text-sm text-muted-foreground">
